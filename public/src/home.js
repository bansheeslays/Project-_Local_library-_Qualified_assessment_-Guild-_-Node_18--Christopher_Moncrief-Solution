function getTotalBooksCount(books) {
  // YOUR SOLUTION HERE
  return books.length; // Returns the total number of book objects within the array
}

function getTotalAccountsCount(accounts) {
  // YOUR SOLUTION HERE
  return accounts.length; // Returns the total number of accounts objects within the array
}
...
function getBooksBorrowedCount(books) {
  ...
  return books.filter((book) => !book.borrows[0].returned).length; // Calculates the total number of books that are currently borrowed using the filter method, adding those to its own array for object totals
}
